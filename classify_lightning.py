...
class LightningWrapper(pl.LightningModule):
    def __init__(self, model, cfg, snapshot_path, train_set, val_set, pretrained, loss, file_name):
        ...
        self.length = self.hparams.Chordmixer.max_len
    ...
    def training_epoch_end(self, outputs):
        ...
        for i in range(self.model_config.output_size):
            rocauc = self.train_auc(train_preds[i], train_labels[i])
    ...
    def validation_epoch_end(self, outputs):
        ...
        for i in range(self.model_config.output_size):
            ...
            if len(val_labels[i]) != 0 and sum(val_labels[i]) != len(val_labels[i]) and sum(val_labels[i]) != 0:
                rocauc = self.val_auc(val_preds[i], val_labels[i])
...
def classify_main(cfg, key):
    ...
    length = cfg.Fine_tuning.Chordmixer.max_len
    model_name = "chordmixer"
    loss = nn.BCEWithLogitsLoss()
    # x = torch.load(f"./data/ref_{length}_{key}_train_new.pt")
    # print(x.shape)

    train_ref = torch.load(f"./data/ref_{length}_{key}_train_new.pt")
    train_alt = torch.load(f"./data/alt_{length}_{key}_train_new.pt")
    train_tissue = torch.load(f"./data/tissue_{length}_{key}_train_new.pt")
    train_label = torch.load(f"./data/label_{length}_{key}_train_new.pt")
